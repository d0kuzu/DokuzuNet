...
namespace DokuzuNet.ServerTest
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            await manager.StartServerAsync(11000);
            ...
            {
                Console.WriteLine($"Player Joined ({player.Connection.EndPoint})");
            };
            manager.OnPlayerLeft += player =>
            {
                Console.WriteLine($"Player Left ({player.Connection.EndPoint})");
            };
